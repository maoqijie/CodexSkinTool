...
2. 点击“一键应用”。如果 Codex 正在运行，确认受控重启。
3. 需要撤销时点击“恢复原始外观”。
...
OpenAI 的 Codex Desktop 设置支持基础主题、强调色、背景/前景色和 UI/代码字体。CodexSkinTool 通过 `~/.codex/config.toml` 的 `[desktop]` 外观配置实现主题切换，不启用 Chromium 调试端口，也不注入桌面渲染器。
...
工具状态位于 `~/Library/Application Support/CodexSkinTool`。完整安全说明见 [SECURITY.md](SECURITY.md)。
...
- **KISS / YAGNI**：首版只实现官方支持的配色与字体主题，不引入 CDP、常驻服务、主题商店或远程资源。
