...
- 自定义主题资料库：将当前配置保存为独立主题，之后可以快速预览和切换。
- 主题管理：内置与自定义主题均可删除；已删除的内置主题可以从设置页恢复。
- 自定义编辑：名称、明暗模式、代码主题、强调色、文字色、背景色与对比度均可调整。
...
1. 在“换肤”页选择主题并即时预览 Codex 界面。
2. 在“设置”页编辑配色或导入背景图片，点击“保存到我的主题”。
3. 回到“换肤”页即可快速切换保存的主题；主题卡右上角可删除。
4. 点击“应用主题”。如果 Codex 正在运行，确认受控重启。
5. 需要撤销时点击“恢复原始外观”。
...
工具状态、主题资料库、自定义草稿和复制后的背景图片位于 `~/Library/Application Support/CodexSkinTool`，文件权限为 `0600`。完整安全说明见 [SECURITY.md](SECURITY.md)。
